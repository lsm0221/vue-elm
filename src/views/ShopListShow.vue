<template>
  <div>
    <div class="shop_home_content">
      <ul>
        <li
          @click.prevent="shopFood(restaurant)"
          class="shop_home_li"
          v-for="(restaurant,index) in restaurants"
          :key="index"
        >
          <div class="shop_home_img">
            <img :src="'http://elm.cangdu.org/img/' + restaurant.image_path" />
          </div>
          <div class="shop_home_right">
            ...
            <h5 class="shop_right_middle">
              <div class="shop_right_middle_l">
                <span class="star">
                  ...
                  <i class="fa fa-star"></i>
                  <i class="fa fa-star"></i>
                  <i class="fa fa-star" v-if="restaurant.rating < 5 ? false : true "></i>
                  <i class="fa fa-star-half-o" v-else></i>
                </span>

                <span class="num">{{restaurant.rating}}</span>
                <b>月售{{restaurant.recent_order_num}}单</b>
              </div>
            ...
            <h5 class="shop_right_footer">
              <p class="send">
                ...
                <span>/</span>
                {{restaurant.piecewise_agent_fee.tips}}
              </p>
              <p class="distance">
                <span>{{restaurant.distance}}</span>
                <span>/</span>
                <span class="orderTime">{{restaurant.order_lead_time}}</span>
              </p>
            </h5>
          </div>
        </li>
...
export default {
  ...
  created() {
    this.getShopHome(31.22967, 121.4762);
  },
